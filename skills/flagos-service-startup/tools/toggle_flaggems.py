...
def generate_env_config(action, output=ENV_CONFIG_PATH):
    """Plugin 场景：生成环境变量配置文件"""
    lines = [
        "#!/bin/bash",
        f"# Auto-generated by toggle_flaggems.py --action {action}",
    ]
    if action == "enable":
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"env_config.sh 路径（plugin 模式，默认 {ENV_CONFIG_PATH}）",
    )
    ...
    # Plugin 模式：生成环境变量配置文件
    if is_plugin and args.action in ("enable", "disable"):
        env_path = generate_env_config(args.action, args.env_config_path)
        result = {
            "action": args.action,
            "mode": "plugin",
            "env_config_path": env_path,
            "success": True,
            "message": f"Plugin 模式: env_config.sh 已生成 ({args.action})",
        ...
        if args.json:
            ...
            print("=" * 50)
            print(f"  env_config.sh: {env_path}")
            print(f"  提示: 重启服务前 source {env_path}")
        return
    ...
    if is_plugin and args.action == "status":
        ...
        result = {
            ...
            "env_config_exists": Path(args.env_config_path).exists(),
        }
        if args.json:
            print(json.dumps(result, indent=2, ensure_ascii=False))
        else:
            ...
            print(f"  env_config.sh 存在: {Path(args.env_config_path).exists()}")
        return
