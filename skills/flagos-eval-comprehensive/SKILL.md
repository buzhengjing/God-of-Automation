...
| 模块 | 用途 | 场景 |
|------|------|------|
| **A — 本地快速评测** | GPQA Diamond 快速评测（fast_gpqa.py） | Quick 模式筛查、迁移流程步骤④⑦ |
| **B — 远端 flageval 正式评测** | 远端 FlagRelease 平台，6 个数据集 | 正式评测、复测阶段 |
| **C — 精度对比（V1 vs V2）** | 对比两版评测结果，5% 阈值判定 | 步骤⑦完成后自动执行 |
...
步骤④（V1 精度）和步骤⑦（V2 精度）使用本模块。

**步骤④ — V1 (Native) 精度**（可选，询问用户）：
```bash
docker exec $CONTAINER bash -c "cd /flagos-workspace/eval && \
    python fast_gpqa.py --config fast_gpqa_config.yaml"
# 保存结果
...
**步骤⑦ — V2 (Full FlagGems) 精度**（强制执行，不可跳过）：
```bash
docker exec $CONTAINER bash -c "cd /flagos-workspace/eval && \
    python fast_gpqa.py --config fast_gpqa_config.yaml"
# 保存结果
...
步骤⑦（V2 精度评测）完成后，如果步骤④（V1 精度评测）有结果，**自动执行对比**。
